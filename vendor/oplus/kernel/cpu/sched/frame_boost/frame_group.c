...
void set_hwui_thread(int grp_id, int pid, int hwtid1, int hwtid2)
{
	...
	if (hwtask1 && hwtask2) {
		...
		if (IS_ERR_OR_NULL(hwots1) || IS_ERR_OR_NULL(hwots2)) {
			rcu_read_unlock();
			goto done;
		}
...
{
	int cpu;
	unsigned long cap = 0, best_cap = 0;
	int max_cap = -1;
...
#else
		cap = oplus_capacity_orig_of(cpu);
#endif
...
{
	unsigned long cpu_capacity = oplus_capacity_orig_of(cpu_of(rq));
	struct frame_group *grp = NULL;
	struct oplus_sched_cluster *cluster = NULL;
	struct oplus_task_struct *ots = NULL;
	unsigned long flags;
...
#else
	return group_task_fits_cluster_cpus(p, cluster) &&
		(cpu_capacity < oplus_capacity_orig_of(cpumask_first(&cluster->cpus)));
#endif
}
...
#else
	return oplus_capacity_orig_of(dst_cpu) < oplus_capacity_orig_of(cpumask_first(&cluster->cpus));
#endif
}
...
#else
	fits = oplus_capacity_orig_of(cpu) >= grp_util;
#endif
...
#else
			oplus_capacity_orig_of(cpu), fits);
#endif
