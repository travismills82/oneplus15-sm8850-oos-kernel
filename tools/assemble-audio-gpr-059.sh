...
# Assemble the bounded Audio .059 GPR teardown candidate over Cellular102 core.

set -euo pipefail

readonly release=6.12.23-android16-5-o-g6744a3f6bcf4-4k
readonly signer='OnePlus 15 Controlled OOS Module Signing v1'
readonly baseline_sha=538622b7d5ff73ab092619cdfab31099ffa3e0638b9051329bf950a04a5260a2
readonly -a expected_resign=(
    adsp_loader_dlkm audio_pkt_dlkm audio_prm_dlkm audpkt_ion_dlkm
    lpass_bt_swr_dlkm lpass_cdc_dlkm lpass_cdc_rx_macro_dlkm
    lpass_cdc_tx_macro_dlkm lpass_cdc_va_macro_dlkm
    lpass_cdc_wsa2_macro_dlkm lpass_cdc_wsa_macro_dlkm machine_dlkm
    mbhc_dlkm oplus_audio_daemon pinctrl_lpi_dlkm spf_core_dlkm
    swr_ctrl_dlkm swr_dmic_dlkm wcd9378_dlkm wcd938x_dlkm
    wcd939x_dlkm wcd9xxx_dlkm
)
...
[[ $(sha256 "$baseline_image") == "$baseline_sha" ]] ||
    die "base image is not the physically qualified Cellular102 vendor_dlkm"
[[ $(modinfo -F name "$replacement") == gpr_dlkm ]] ||
    die "replacement is not gpr_dlkm"
[[ -z $(modinfo -F signer "$replacement") ]] ||
    die "GPR build output must be unsigned"
[[ $(modinfo -F vermagic "$replacement") == "$release "* ]] ||
    die "GPR build output does not inherit the frozen kernel release"
...
python3 "$repo_root/tools/validate-matched-wlan-vendor-dlkm.py" \
    ...
    --allow-import-contract-change gpr_dlkm \
...
jq -e '
  ...
  .source_replacements == 1 and .protected_export_signed_closure == 23 and
  .re_sign_stock_modules == 22 and .retained_external_modules == 525 and
  .allowed_import_contract_changes == ["gpr_dlkm"] and
...
' "$contract/summary.json" >/dev/null || die "bounded Audio GPR graph failed"
mapfile -t actual_resign < <(
    awk -F '\t' 'NR > 1 && $2 == "RE_SIGN_STOCK" { print $1 }' \
        "$contract/protected-export-signing-closure.tsv" | LC_ALL=C sort
)
mapfile -t reviewed_resign < <(printf '%s\n' "${expected_resign[@]}" | LC_ALL=C sort)
[[ "${actual_resign[*]}" == "${reviewed_resign[*]}" ]] ||
    die "protected signing closure differs from reviewed Audio consumer set"
...
"$repo_root/tools/stage-matched-wlan-vendor-dlkm.sh" \
    ...
    --out-dir "$stage_out" \
    --strip-unneeded-resign wcd9378_dlkm \
    --strip-unneeded-resign wcd939x_dlkm \
    --replacement "$replacement"
...
cp "$repo_root/docs/validation/audio-modernization/source-delta.md" \
    "$out_dir/audio-059-source-delta.md"
cp "$repo_root/docs/validation/audio-modernization/current-audio-closure.tsv" \
    "$out_dir/current-audio-closure.tsv"
cp "$repo_root/docs/validation/audio-modernization/candidate-ranking.tsv" \
    "$out_dir/audio-candidate-ranking.tsv"

python3 "$repo_root/tools/verify-audio-gpr-059-candidate.py" \
...
python3 - "$repo_root/tools/controlled-v1-wlan053-kernel-contract.json" \
    "$repo_root/out/audio-gpr-059/build-contract.txt" "$out_dir/release-contract.json" <<'PY'
...
manifest = {
    "schema_version": 2,
    "name": "controlled-v1-audio059-gpr-teardown",
    ...
    "subsystems": {
        ...
        "audio": {
            "baseline_generation": ".046",
            "candidate_generation": ".059 selective GPR teardown",
            "baseline_source_id": build["audio_current_source_id"],
            "candidate_source_id": build["audio_candidate_source_id"],
        },
    },
    "signing": {
...
{
    printf 'Audio .059 selective GPR candidate: PASS\n'
    ...
    printf 'source_upgrades=1:gpr_dlkm\n'
    printf 'exact_stock_resign=22\n'
    printf 'unexpected_module_changes=0\n'
    printf 'gpr_export_changes=0\n'
    printf 'gpr_import_change=cancel_work_sync:0x35480a5b:REVIEWED\n'
    ...
    printf 'non_runtime_elf_delta=wcd9378_dlkm,wcd939x_dlkm:STRIP_UNNEEDED_SYMBOLS_ONLY\n'
    printf 'physical_flash=NOT_PERFORMED\n'
} > "$out_dir/validation-report.txt"

(
    cd "$out_dir"
    sha256sum vendor_dlkm.img release-contract.json audio-replacement-manifest.tsv \
        audio-gpr-import-crc.tsv audio-gpr-elf-contract.tsv \
        audio-gpr-module-contract.tsv audio-signature-report.tsv \
        audio-preservation-report.tsv system-dlkm-load-contract.tsv \
        validation-report.txt e2fsck-repair.txt e2fsck-pre-avb.txt \
        e2fsck.txt > SHA256SUMS
)
printf 'CONTROLLED AUDIO .059 GPR STATIC CANDIDATE PASS\n'
