...
# Assemble the bounded Platform CoreSight ETR candidate over Camera073 RER.

set -euo pipefail

readonly release=6.12.23-android16-5-o-g6744a3f6bcf4-4k
readonly signer='OnePlus 15 Controlled OOS Module Signing v1'
readonly baseline_sha=8623653732b1f23a8250badc23bb0d99b25b812b5e8afc4ddcb9875154662531
...
[[ $(sha256 "$baseline_image") == "$baseline_sha" ]] ||
    die "base image is not the physically qualified Camera073 RER vendor_dlkm"
[[ $(modinfo -F name "$replacement") == coresight_tmc ]] ||
    die "replacement is not coresight_tmc"
[[ -z $(modinfo -F signer "$replacement") ]] ||
    die "CoreSight TMC build output must be unsigned"
[[ $(modinfo -F vermagic "$replacement") == "$release "* ]] ||
    die "CoreSight TMC build output does not inherit the frozen kernel release"
...
' "$contract/summary.json" >/dev/null || die "bounded Platform module graph failed"
...
cp "$repo_root/docs/validation/platform-modernization/source-delta.md" \
    "$out_dir/platform-099064-to-099086-source-delta.md"
cp "$repo_root/docs/validation/platform-modernization/platform-099064-to-099086-audit.tsv" \
    "$out_dir/platform-099064-to-099086-audit.tsv"
cp "$repo_root/docs/validation/platform-modernization/candidate-ranking.tsv" \
    "$out_dir/platform-candidate-ranking.tsv"

python3 "$repo_root/tools/verify-platform-coresight-etr-candidate.py" \
...
python3 - "$repo_root/tools/controlled-v1-wlan053-kernel-contract.json" \
    "$repo_root/out/platform-coresight-etr-086/build-contract.txt" \
...
manifest = {
    "schema_version": 2,
    "name": "controlled-v1-platform099086-selective-coresight-etr",
    ...
    "subsystems": {
        ...
        "display": ".071 retained; .097 deferred",
        "camera": ".073 selective RER physically qualified and retained",
        "platform": {
            "baseline_generation": ".099.064",
            "candidate_generation": ".099.086 selective CoreSight ETR UAF guard",
            "baseline_source_id": build["platform_current_source_id"],
            "candidate_source_id": build["platform_candidate_source_id"],
            "selected_source_commit": build["selected_source_commit"],
        },
    },
    "signing": {
...
{
    printf 'Platform .099.086 selective CoreSight ETR candidate: PASS\n'
    ...
    printf 'dist=PASS\nABI=PASS_EMPTY\nKMI=PASS\n'
    printf 'vendor_modules=436\nsource_upgrades=1:coresight_tmc\n'
    printf 'unexpected_module_changes=0\n'
    printf 'coresight_export_changes=0\ncoresight_import_changes=0\n'
...
(
    ...
    sha256sum vendor_dlkm.img release-contract.json \
        platform-replacement-manifest.tsv platform-import-crc.tsv \
        platform-module-contract.tsv platform-signature-report.tsv \
        platform-preservation-report.tsv system-dlkm-load-contract.tsv \
        validation-report.txt e2fsck-repair.txt e2fsck-pre-avb.txt \
        e2fsck.txt > SHA256SUMS
)
printf 'CONTROLLED PLATFORM .099.086 CORESIGHT ETR STATIC CANDIDATE PASS\n'
