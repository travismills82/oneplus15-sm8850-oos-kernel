...
function build_system_dlkm() {
  ...
  if [[ ${SYSTEM_DLKM_RE_SIGN:-1} == "1" ]]; then
    # Kleaf's public module_signing_key attribute restores a declared key at
    # $OUT_DIR using CONFIG_MODULE_SIG_KEY's basename.  The historical fixed
    # certs/signing_key.pem path only works for Kbuild's autogenerated key,
    # and would either reject a declared key or tempt callers to create an
    # undeclared private-key copy.  Resolve the configured key exactly as the
    # kernel build does, and fail closed if it is not present in this action.
    local module_sig_key
    local system_dlkm_signing_key
    module_sig_key=$(sed -n 's/^CONFIG_MODULE_SIG_KEY="\([^"]*\)"$/\1/p' "${OUT_DIR}/.config")
    if [[ -z "${module_sig_key}" ]]; then
      echo "ERROR: CONFIG_MODULE_SIG_KEY is required to sign system_dlkm modules" >&2
      exit 1
    fi

    if [[ "${module_sig_key}" == pkcs11:* ]]; then
      system_dlkm_signing_key="${module_sig_key}"
    elif [[ "${module_sig_key}" == /* ]]; then
      system_dlkm_signing_key="${module_sig_key}"
    elif [[ -f "${OUT_DIR}/${module_sig_key}" ]]; then
      system_dlkm_signing_key="${OUT_DIR}/${module_sig_key}"
    elif [[ -f "${ROOT_DIR}/${module_sig_key}" ]]; then
      system_dlkm_signing_key="${ROOT_DIR}/${module_sig_key}"
    else
      echo "ERROR: configured module signing key is unavailable: ${module_sig_key}" >&2
      exit 1
    fi

    if [[ ! -f "${OUT_DIR}/certs/signing_key.x509" ]]; then
      echo "ERROR: module signing certificate is unavailable in ${OUT_DIR}" >&2
      exit 1
    fi

    for module in $(find ${SYSTEM_DLKM_STAGING_DIR} -type f -name "*.ko"); do
      ${OUT_DIR}/scripts/sign-file sha1 \
      "${system_dlkm_signing_key}" \
